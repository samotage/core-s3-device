...
void AppMic::Update() {
    if (Model.IsRecording()) {
        Model.WriteChunk();
        View.SetRecording(true, Model.RecordingSeconds(), Model.LastFilename());
    }
...
void AppMic::onEvent(lv_event_t* event) {
    ...
    if (obj == instance->_root) {
        ...
        if (code == LV_EVENT_CLICKED) {
            ...
            } else if (obj == instance->View.ui.btn_record) {
                if (instance->Model.IsRecording()) {
                    instance->Model.StopRecording();
                    instance->View.SetRecording(false, 0, instance->Model.LastFilename());
                } else {
                    if (instance->Model.StartRecording()) {
                        instance->View.SetRecording(true, 0, instance->Model.LastFilename());
                    } else {
                        lv_label_set_text(instance->View.ui.label_status, "SD card error");
                    }
                }
            }
        }
